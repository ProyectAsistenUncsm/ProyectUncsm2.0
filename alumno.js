...
function listar() {
    tabla = $('#tbllistado').DataTable({
        "ajax": {
            url: 'Alumnos_controlador.php?op=listar',
            type: "GET",
            dataType: "json",
...
function guardaryeditar(e) {
    ...
    $.ajax({
        url: 'Alumnos_controlador.php?op=guardaryeditar',
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function desactivar(alumno_id) {
    bootbox.confirm("¿Está seguro de desactivar este dato?", function(result) {
        if (result) {
            $.post("Alumnos_controlador.php?op=desactivar", { alumno_id: alumno_id }, function(e) {
                bootbox.alert(e);
                tabla.ajax.reload();
            });
        }
...
function activar(alumno_id) {
    bootbox.confirm("¿Está seguro de activar este dato?", function(result) {
        if (result) {
            $.post("Alumnos_controlador.php?op=activar", { alumno_id: alumno_id }, function(e) {
                bootbox.alert(e);
                tabla.ajax.reload();
            });
        }
